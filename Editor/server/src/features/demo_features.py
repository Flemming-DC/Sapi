...
_COLOR = re.compile(r"""\#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})(?!\w)""")
_ADDITION = re.compile(r"^\s*(\d+)\s*\+\s*(\d+)\s*=(?=\s*$)")

class CallBacks:

    @server.feature(types.TEXT_DOCUMENT_DOCUMENT_COLOR)
    def _document_color(params: types.CodeActionParams):
        ...
        for linum, line in enumerate(document.lines):
            for match in _COLOR.finditer(line.strip()):
    ...
    def _color_presentation(params: types.ColorPresentationParams):
    ...
    # def _code_actions(params: types.CodeActionParams):
    #     items = []
    #     document_uri = params.text_document.uri
    #     document = server.workspace.get_text_document(document_uri)
