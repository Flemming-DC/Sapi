...
def inlay_hints(params: t.InlayHintParams) -> list[t.InlayHint]:
    ...
    r = params.range
    sections = server.sapi_sections(uri, True) # Range has been is ignored. Bring it back. [r.start.line : r.end.line + 1]
...
def inlay_hints_work(sections: list[Section]) -> list[t.InlayHint]:
    ...
    sapi_code = sections[idx].leading_whitespace + sections[idx].query # temp
    lines = [line for line in sapi_code.split(os.linesep)] 
    sapi_code_os_sep = os.linesep.join([line.strip('\r\n') for line in lines]) # does this work for multiple queries?
    
    dataModel = settings.load_datamodel()
    try:
        sql_token_trees = sapi.parse(sapi_code_os_sep, dataModel, list[TokenTree])
    except Exception as e: # create a user error class for the parser and filter by it
        code_start = sapi_code_os_sep if len(sapi_code_os_sep) < 10 else sapi_code_os_sep[:10] + '...'
