...
class CustomTitleBar(QWidget):
    def __init__(self, parent: QWidget):
        ...
        min_icon = QIcon()
        min_icon.addFile(_icon_folder + "min.svg")
        ...
        max_icon = QIcon()
        max_icon.addFile(_icon_folder + "max.svg")
        ...
        close_icon = QIcon()
        close_icon.addFile(_icon_folder + "close.svg")  # Close has only a single state.
        ...
        normal_icon = QIcon()
        normal_icon.addFile(_icon_folder + "normal.svg")
    ...
    def window_state_changed(self, state: Qt.WindowStates):
...
class MainWindow(QMainWindow):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton:
            self.initial_pos = event.pos() # position().toPoint() -> pos()
        super().mousePressEvent(event)
        event.accept()

    def mouseMoveEvent(self, event: QMouseEvent):
        if self.initial_pos is not None:
            delta: QPoint = event.pos() - self.initial_pos  # position().toPoint() -> pos()
            self.window().move(
